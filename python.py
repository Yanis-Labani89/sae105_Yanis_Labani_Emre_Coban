...
def charger_donnees(chemin_fichier):
    ...
    with open(chemin_fichier, mode='r') as fichier_csv:  
        ...
        entete = next(lecteur)  # Ignore la première ligne (en-tête)
        for ligne in lecteur:  
            try:
                altitude = float(ligne[4])  # Récupère la valeur d'altitude (colonne 5)
                precipitation = float(ligne[6])  # Récupère la valeur de précipitation (colonne 7)
...
donnees_combinees = donnees_59 + donnees_05 + donnees_75  
